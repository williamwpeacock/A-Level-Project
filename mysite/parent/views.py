...
class WelcomeView(TemplateView):
    template_name = 'parent/welcome.html'
        ...
        if user.is_authenticated:
            ...
            if user.role == "Parent":
                ...
                if len(parents) == 1:
                    ...
                    # Gets all students related to this person
                    # Gets this person's bookings
                    args = {'username': user.username, 'fullname': parent.fullname, 'role': user.role}
                    args['children'] = Student.objects.filter(parent=parent).values('id', 'fullname')
                    args['bookings'] = PEBooking.objects.filter(student__parent=parent).values('student__fullname', 'parentsevening__pename', 'schoolclass__subject', 'parentsevening__date', 'timeslot').order_by('parentsevening__date', 'timeslot')
        ...
        if user.is_authenticated:
            ...
            if user.role == "Parent":
                ...
                if len(parents) == 1:
                    ...
                    # Checks the current student exists and it is related to this person
                    # Gets the current student
                    students = Student.objects.filter(Q(id=student_id) & Q(parent=parent))
                    if len(students) == 1:
                        student = students[0]

                        # Creates the dictionary to be passed to the template
                        # Gets all parents' evenings applicable to this student
                        # Gets this student's bookings
                        args = {'username': user.username, 'role': user.role, 'student': student}
...
class ParentsEveningView(TemplateView):
    ...
    def get(self, request, student_id, pe_id):
        user = request.user
                ...
                if len(parents) == 1:
                    ...
                    if len(students) == 1:
                        ...
                        if len(pes) == 1:
                            ...
                            # Creates the dictionary to be passed to the template
                            # Gets all classes applicable to this student and parentsevening
                            # Gets this student's bookings in this parents' evening
                            args = {'username': user.username, 'role': user.role, 'student': student, 'pe': pe}
                            ...
                            return redirect('parent:child', student.id)
                    else:
                        return redirect('parent:welcome')
                else:
                    return redirect('login:logout')
...
class ClassView(TemplateView):
    ...
    def get(self, request, student_id, pe_id, class_id):
        user = request.user
                    ...
                    if len(students) == 1:
                        ...
                        if len(pes) == 1:
                            ...
                            if len(schoolclasses) == 1:
                                ...
                                # Creates the dictionary to be passed to the template
                                # Gets all classes applicable to this student and parentsevening
                                args = {'username': user.username, 'role': user.role, 'student': student, 'pe': pe, 'schoolclass': schoolclass}
                                args['schoolclasses'] = SchoolClass.objects.filter(Q(students=student) & Q(parentsevening=pe)).values('id', 'subject')
                            ...
                            return redirect('parent:child', student_id)
                    else:
                        return redirect('parent:welcome')
                else:
                    return redirect('login:logout')
...
class BookView(TemplateView):
    ...
    def get(self, request, student_id, pe_id, class_id, timeslot):
        user = request.user
                        ...
                        if len(pes) == 1:
                            ...
                            if len(schoolclasses) == 1:
                                ...
                                if self.time_free(urltotime, pe, schoolclass, student):
                                    ...
                                    # Gets all classes applicable to this student and parentsevening
                                    args = {'username': user.username, 'role': user.role, 'student': student, 'pe': pe, 'schoolclass': schoolclass, 'form': form}
                                    ...
                                    return redirect('parent:class', student_id, pe_id, class_id)
                            else:
                                return redirect('parent:parentsevening', student_id, pe_id)
                        else:
                            return redirect('parent:child', student_id)
                    else:
                        return redirect('parent:welcome')
    ...
    def post(self, request, student_id, pe_id, class_id, timeslot):
        user = request.user
                        ...
                        if len(pes) == 1:
                            ...
                            if len(schoolclasses) == 1:
                                ...
                                if self.time_booked(urltotime, booked_by_me):
                                    ...
                                    # Passes a displayable timeslot to the dictionary
                                    # Gets all classes applicable to this student and parentsevening
                                    args = {'username': user.username, 'role': user.role, 'student': student, 'pe': pe, 'schoolclass': schoolclass, 'notes': booking.notes, 'urltimeslot': timeslot}
                                    args['timeslot'] = timeslot[0]+timeslot[1]+":"+timeslot[2]+timeslot[3]
                                    args['schoolclasses'] = SchoolClass.objects.filter(Q(students=student) & Q(parentsevening=pe)).values('id', 'subject')
